...
import { DOCS_SECTIONS } from "@/app/(shell)/docs/constants";
...
export function DocsSidebar() {
  ...
  return (
    <aside className="sticky top-4 max-h-[calc(100vh-2rem)] overflow-y-auto border border-border/60 bg-card/70 p-3">
      <p className="px-2 pb-2 text-xs font-semibold uppercase tracking-label-sm text-muted-foreground">
        Documentation
      </p>
      <nav
        className="ml-1 space-y-1 border-l border-border/80 pl-2"
        ...
        {DOCS_SECTIONS.map((item) => {
          ...
          return (
            <Link
              ...
              href={href}
              className={clsx(
                "-ml-2 block border-l-2 px-3 py-2 text-sm transition-colors",
                isActive
