...
export function ChatResponseHealthCard({
  ...
  const status = useMemo(() => {
    ...
    if (healthyPct >= 90 && inactive === 0) return { label: "Optimal", color: "text-success", bgColor: "bg-success/12", level: 5, fillClass: "fill-success" };
    if (healthyPct >= 75) return { label: "Good", color: "text-success", bgColor: "bg-success/12", level: 4, fillClass: "fill-success" };
    if (latePct > 15 || inactive > 0) return { label: "Attention Needed", color: "text-warning", bgColor: "bg-warning/12", level: 2, fillClass: "fill-warning" };
    return { label: "Fair", color: "text-warning", bgColor: "bg-warning/12", level: 1, fillClass: "fill-warning" };
    ...
    <div className="flex h-full flex-col rounded-xl bg-card p-4 shadow-card">
      ...
      <div className="flex items-start justify-between gap-2 mb-4">
        ...
        <div className="flex items-center gap-2">
          <span className={cn("font-semibold tabular-nums leading-none", status.color)}>
            <span className="text-xxxs leading-none">{status.label}</span>
          </span>
      ...
      <div className="flex flex-1 flex-col">
        {/* ── Section Label ── */}
        <div className="mb-3">
          <h3 className="text-xxxs font-semibold text-muted-foreground uppercase tracking-wider">
        ...
        <div className="flex flex-col gap-2 mt-auto">
          <div className="flex items-center justify-between rounded-xl bg-muted/50 px-4 py-2">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xxs font-medium text-muted-foreground">Healthy</span>
            </div>
          ...
          <div className="flex items-center justify-between rounded-xl bg-muted/50 px-4 py-2">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xxs font-medium text-muted-foreground">Late (&gt;24h)</span>
            </div>
            <span className="text-sm font-semibold text-foreground">{late}</span>
          </div>

          <div className="flex items-center justify-between rounded-xl bg-muted/50 px-4 py-2">
            <div className="flex items-center gap-3">
              <span className="h-2 w-2 rounded-full bg-destructive" />
              <span className="text-xxs font-medium text-muted-foreground">Unreplied (&gt;5d)</span>
            </div>
