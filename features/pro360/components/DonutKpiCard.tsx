      ...
      <div className="flex h-full flex-col rounded-xl bg-card p-4 shadow-card">
        ...
        <div className="flex items-center justify-between gap-2 mb-4">
          ...
          {trend != null && (
            <span
              className={cn(
                "flex shrink-0 items-center gap-1 font-semibold tabular-nums leading-none",
                TREND_STYLE[trendDirection]
              )}
            >
              <span className="text-xxxs leading-none">{trend}</span>
              <span className="text-xxxs leading-none">{TREND_ARROW[trendDirection]}</span>
            </span>
          )}
        </div>
          ...
          {groups.map((group, gIdx) => {
            ...
            return (
              <div key={gIdx} className="flex flex-1 flex-col min-w-0">
                {group.title && (
                  <div className="mb-3">
                    <span className="text-xxxs font-semibold text-muted-foreground uppercase tracking-wider text-left">
                      {group.title}
                    </span>
                  </div>
                )}
                ...
                <div className="flex flex-col items-center w-full">
                  ...
                  <div className="mt-auto flex w-full max-w-[118px] flex-col gap-2">
                    {group.segments.map((seg, i) => (
                      <div key={i} className="grid w-full grid-cols-[auto_1fr_auto] items-center gap-x-2">
                        <div
                          className="h-1.5 w-1.5 shrink-0 rounded-full"
                          style={{ backgroundColor: seg.color }}
                        />
                        <span className="text-xxs font-medium text-muted-foreground whitespace-nowrap">
                          {seg.label}
                        </span>
                        <span className="pl-2 text-xxs font-normal tabular-nums whitespace-nowrap text-right text-foreground">
                          {Math.round(seg.value).toLocaleString()} h
                        </span>
                      </div>
                    ))}
