...
import { pods, podHistory, type Pod, type PodHistoryEntry } from "@/lib/mock/pods";
...
const DeactivatePodDialog = dynamic(
  () =>
    import("@/components/team/DeletePodDialog").then(
...
export default function PodDetailPage() {
  ...
  const podName = podOrPlaceholder?.name ?? "";
  ...
  useEffect(() => {
    if (podOrPlaceholder) {
      setItems([
        ...
        { label: podOrPlaceholder.name },
      ]);
    }
  }, [podName, setItems, podOrPlaceholder]);

  const currentMemberIds = useMemo(() => {
    if (!podOrPlaceholder) return new Set<string>();
    return new Set(podOrPlaceholder.members.map((m) => m.tfpId));
  }, [podOrPlaceholder]);
  ...
  const paginatedMembers = useMemo(() => {
    ...
    return podOrPlaceholder.members.slice(start, start + membersPageSize);
  }, [podOrPlaceholder, membersPage, membersPageSize]);
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="flex items-center gap-2">
                <CardTitle className="text-lg">{podOrPlaceholder.name}</CardTitle>
        ...
        <CardContent className="space-y-4">
          {podOrPlaceholder.notes && (
            <p className="text-sm text-muted-foreground">{podOrPlaceholder.notes}</p>
          )}
          <div className="text-xs text-muted-foreground">
            Created {new Date(podOrPlaceholder.createdAt).toLocaleString("en-GB")} · Last edited{" "}
            {new Date(podOrPlaceholder.updatedAt).toLocaleString("en-GB")}
          </div>
        </CardContent>
      </Card>
      ...
      {notificationSent && (
        <div className="flex items-center gap-2 rounded-lg border border-primary/30 bg-primary/5 px-4 py-2 text-sm text-foreground">
          ...
          Notification sent to pod members (mock).
        </div>
      )}
      ...
      <div className="space-y-3">
        ...
        <Card>
          <CardContent className="p-0">
            ...
            {podOrPlaceholder.members.length === 0 ? (
              <div className="px-4 py-8 text-center text-sm text-muted-foreground">
                No members yet. Add members using the button above.
              </div>
            ) : (
              <TablePagination
                total={podOrPlaceholder.members.length}
      ...
      {deactivateOpen && (
        <DeactivatePodDialog
          open={deactivateOpen}
          podName={podOrPlaceholder.name}
          memberCount={podOrPlaceholder.members.length}
          onOpenChange={(open) => {
