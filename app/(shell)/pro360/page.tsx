...
function AppointmentChartLegend() {
  return (
    <div className="mt-2 flex flex-wrap justify-center gap-x-5 gap-y-2 text-xs text-muted-foreground px-2">
      <div className="flex items-center gap-3">
        <span className="text-micro font-semibold uppercase tracking-wide text-foreground/50">Appointments</span>
      ...
      <div className="flex items-center gap-3">
        <span className="text-micro font-semibold uppercase tracking-wide text-foreground/50">Case Notes</span>
                  ...
                  <div className="flex -space-x-1 overflow-hidden">
                    {atRiskPros.slice(0, 3).map((p) => (
                      <div key={p.id} className="relative inline-block h-5 w-5 rounded-full ring-1 ring-background bg-muted">
                        {p.avatar ? (
                          ...
                          <span className="flex h-full w-full items-center justify-center rounded-full text-micro font-medium text-muted-foreground">
                            {p.name.charAt(0)}
                          </span>
                        )}
                      </div>
                    ...
                    {atRiskPros.length > 3 && (
                      <div className="relative inline-flex h-5 w-5 items-center justify-center rounded-full bg-muted ring-1 ring-background">
                        <span className="text-micro font-medium text-muted-foreground">+{atRiskPros.length - 3}</span>
                      </div>
                    )}
                  </div>
                )}
          ...
          <KpiCard
            ...
            value={
              <>
                ...
                <span className="ml-1 inline-flex items-center rounded-full bg-muted px-2 py-1 text-micro font-medium text-muted-foreground">
                  {tfPlusCount} TF+
                </span>
              </>
            }
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <Card className="flex h-full flex-col">
            ...
            <CardContent className="flex-1 pt-0">
              <ChartContainer
                config={chatHoursChartConfig}
                className="h-[200px] w-full [&_.recharts-cartesian-axis-tick_text]:text-xxxs"
              >
                <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0} initialDimension={{ width: 600, height: 200 }}>
          ...
          <Card className="flex h-full flex-col">
            ...
            <CardContent className="flex-1 pt-0">
              <ChartContainer
                config={therapyHoursChartConfig}
                className="h-[200px] w-full [&_.recharts-cartesian-axis-tick_text]:text-xxxs"
              >
                <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0} initialDimension={{ width: 600, height: 200 }}>
        ...
        {podiumOrder.map((p, idx) => {
          ...
          return (
            <div key={p.id} className={cn(
              ...
              isFirst ? "bg-warning/8 dark:bg-warning/12 scale-110 -translate-y-2 border border-warning/25 min-w-[130px]" : "bg-muted/30 min-w-[110px]"
            )}>
              <div className={cn(
                "flex h-6 w-6 items-center justify-center rounded-full text-xxxs font-bold",
                rank === 1 ? "bg-warning text-white" : rank === 2 ? "bg-muted text-muted-foreground" : "bg-warning/60 text-white"
              )}>
                {rank}
              </div>
              ...
              <div className="min-w-0">
                ...
                <div className="mt-1 flex flex-col items-center">
                  <span className="text-xxxs font-bold text-warning tabular-nums">#{speedRank} speed</span>
                  <span className="text-micro text-muted-foreground uppercase opacity-70">{p.avgServiceResponseHours}h avg</span>
                </div>
              </div>
            </div>
          );
        ...
        <Table>
          <TableHeader>
            <TableRow className="hover:bg-transparent">
              ...
              <TableHead className="text-center">Speed Rank</TableHead>
          ...
          <TableBody>
            {others.map((p, i) => {
              ...
              return (
                <TableRow key={p.id}>
                  <TableCell className="text-xs font-medium text-muted-foreground tabular-nums">{overallRank}</TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2 min-w-0">
                      {p.avatar ? (
                        <Image src={p.avatar} alt="" width={24} height={24} className="rounded-full shadow-sm" />
                      ) : (
                        <div className="h-6 w-6 rounded-full bg-muted flex items-center justify-center text-micro uppercase">
                          {p.name.charAt(0)}
                        </div>
                      )}
                      <div className="min-w-0">
                        <p className="truncate text-sm font-medium text-foreground">{p.name}</p>
                        <p className="text-micro text-muted-foreground tabular-nums">{p.responseRate12h}% chat</p>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell className="text-center">
                    <div className="flex flex-col items-center">
                      <span className="text-xs font-semibold text-warning tabular-nums">#{speedRank}</span>
                      <span className="text-micro text-muted-foreground opacity-70 tabular-nums">{p.avgServiceResponseHours}h</span>
                    </div>
                  </TableCell>
                  <TableCell className="text-center"><CheckCircle className="mx-auto h-3.5 w-3.5 text-success" /></TableCell>
                  <TableCell className="text-center text-xs font-semibold text-foreground tabular-nums">{p.msgCount}</TableCell>
                </TableRow>
              );
            })}
          </TableBody>
