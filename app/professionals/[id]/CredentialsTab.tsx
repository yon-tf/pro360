...
export function CredentialsTab({ professionalId }: { professionalId: string }) {
  const [records, setRecords] = useState(() =>
    credentialRecords.filter((c) => c.professionalId === professionalId)
  );
  const [history, setHistory] = useState<CredentialHistoryEntry[]>(() =>
    credentialHistory.filter((h) =>
      credentialRecords.some((c) => c.professionalId === professionalId && c.id === h.credentialId)
    )
  );
  ...
  function handleAdd() {
    ...
    if (newRecord.fileName) {
      setFileUrls((prev) => ({
        ...prev,
        [newId]: `https://mock-storage.local/credentials/${newId}/${newRecord.fileName}`,
      }));
    }
    setHistory((prev) => [
      ...prev,
  ...
  function handleEdit() {
    ...
    setRecords((prev) =>
      prev.map((c) =>
        c.id === editId
          ? {
              ...c,
              type: (form.type as CredentialType) || c.type,
              ...
              status: calculateCredentialStatus(form.expiryDate || null),
              notes: form.notes,
              fileName: form.fileName || c.fileName,
              updatedAt: now,
            }
          : c
      )
    ...
    if (form.fileName) {
      setFileUrls((prev) => ({
        ...prev,
        [editId]: `https://mock-storage.local/credentials/${editId}/${form.fileName}`,
      }));
    }
    setHistory((prev) => [
      ...prev,
  ...
  function handleRenew() {
    ...
    setRecords((prev) => [
      ...prev.map((c) => (c.id === renewId ? { ...c, archived: true, updatedAt: now } : c)),
      {
        id: newId,
        professionalId,
        type: "Contract",
    ...
    if (form.fileName) {
      setFileUrls((prev) => ({
        ...prev,
        [newId]: `https://mock-storage.local/credentials/${newId}/${form.fileName}`,
      }));
    }
    setHistory((prev) => [
      ...prev,
  ...
  function handleArchive(id: string) {
    const now = new Date().toISOString();
    setRecords((prev) =>
      prev.map((c) => (c.id === id ? { ...c, archived: true, updatedAt: now } : c))
    );
    setHistory((prev) => [
      ...prev,
      {
