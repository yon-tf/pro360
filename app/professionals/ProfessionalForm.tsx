...
} from "@/lib/mock/professionalProfileStore";
...
export function ProfessionalForm({
  ...
  function handleSave() {
    ...
    systemToast.success(
      mode === "create" ? "Professional successfully created" : "Professional successfully updated",
  ...
  const backHref = mode === "edit" && profile ? `/professionals/${profile.id}/profile` : "/professionals";

  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <Button variant="ghost" size="sm" asChild>
          <Link href={backHref}><ChevronLeft className="h-4 w-4" />Back</Link>
        </Button>
        <Button onClick={handleSave}>
          <Save className="h-4 w-4" />
          {mode === "create" ? "Create professional" : "Save changes"}
        </Button>
      </div>
        ...
        <CardContent className="space-y-4 pt-2">
          <div className="flex items-center justify-between rounded-lg border border-border px-5 py-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Visible to clients when enabled</p>
            </div>
      ...
      <Card className="border-destructive/40">
        ...
        <CardContent>
          <div className="flex items-center justify-between rounded-lg border border-amber-300 bg-amber-50 dark:bg-amber-950/20 px-5 py-4">
            <div>
              <p className="text-sm font-medium">Departure date</p>
              <p className="text-xs text-muted-foreground mt-0.5">
                Set the date this professional departed the platform. This will deactivate their profile.
              </p>
            </div>
            <DatePicker
