...
import {
  Search, Building2, Users, Star, Sparkles, Zap, BarChart3, DollarSign,
...
export function QualityByOrganization() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3 md:grid-cols-4 xl:grid-cols-7">
        <KpiCard title="Total Organizations" value={headerMetrics.totalOrgs} icon={<Building2 className="h-5 w-5" />} trend="+1" trendDirection="up" />
        <KpiCard title="Total Professionals" value={headerMetrics.totalPros} icon={<Users className="h-5 w-5" />} trend="+3" trendDirection="up" />
        <KpiCard title="Avg Customer Rating" value={<>{headerMetrics.avgCustomerRating} <span className="text-sm font-normal text-muted-foreground">out of 5</span></>} icon={<Star className="h-5 w-5" />} trend="+0.1" trendDirection="up" />
        <KpiCard title="Avg AI Rating" value={<>{headerMetrics.avgAiRating} <span className="text-sm font-normal text-muted-foreground">out of 5</span></>} icon={<Sparkles className="h-5 w-5" />} badge="Stable" />
        <KpiCard title="Total Activations" value={headerMetrics.totalActivations} icon={<Zap className="h-5 w-5" />} trend="+4" trendDirection="up" />
        <KpiCard title="Activation Rating" value={<>{headerMetrics.avgActivationRating} <span className="text-sm font-normal text-muted-foreground">out of 5</span></>} icon={<BarChart3 className="h-5 w-5" />} trend="-0.1" trendDirection="down" />
        <KpiCard title="Total Payout" value={`$${headerMetrics.totalPayout.toLocaleString()}`} icon={<DollarSign className="h-5 w-5" />} trend="+8%" trendDirection="up" />
      </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-right">OMWA</TableHead>
                    <TableHead className="text-center">No. of TFP</TableHead>
                    <TableHead className="text-right">Customer Rating</TableHead>
                    <TableHead className="text-right">AI Rating</TableHead>
                    <TableHead className="text-right">Activation Rating</TableHead>
                ...
                <TableBody>
                  ...
                  {paginatedOrgs.length === 0 && (
                    <TableRow>
                      <TableCell colSpan={7} className="py-8 text-center text-sm text-muted-foreground">No organizations found.</TableCell>
                    </TableRow>
                  )}
                </TableBody>
              </Table>
